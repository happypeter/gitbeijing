...
补充一句。要删除一个仓库，就到左侧列表的项目名字上，右击，然后点 `Remove` 就行了。

![](images/github_for_mac/remove_repo.png)


### 基本版本控制
第二步，编辑项目。这个就用自己最喜欢的编辑器，我用 sublime，打开 GUI 这个项目，创建一个 README 文件吧，里面随便写点内容就行。

第三步，commit。 commit 英文的基本意思是执行某个重要的事情，例如 commit suicide，自我了断。但是在 git 这里，做动词讲的时候就是“保存版本”，当名词讲就是版本。所以这一步里也就是真正保存一个版本了，这个就关键了。
