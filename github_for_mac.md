...
既然历史版本都已经保存了，那么我要时空穿梭，回到任何一个版本的状态应该也是可以的。比如我刚刚做了一个版本，就发现修改的内容有问题，不想要这个版本了。可以到 `Changes` 选项下，页面最底端的 `undo` 标签，一点，这次版本就没了，注意，undo 操作是对尚未 `Unsynced` 的版本有效，如果版本已经同步到远端，也就是大家已经都看见了你的操作，那么是不适合直接删除这个版本的。另外就是要注意，随着版本被 undo，所有你的修改内容也都跟着没了。
